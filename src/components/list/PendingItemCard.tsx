...
const PendingItemCard = ({item} :{item:GroceryItem}) => {
    ...
    const priorityPillBg = {
      ...
      return (
        <View className="rounded-3xl border border-primary bg-card-foreground p-4">
          <View className="flex-row items-start gap-3">
            <Pressable
              className="mt-1 size-6 items-center justify-center rounded-full border-2 border-secondary-foreground "
              onPress={() => togglePurchased(item.id)}
            ></Pressable>
    
            <View className="flex-1">
              <View className="flex-row items-center justify-between gap-2">
                <Text className="flex-1 text-lg font-semibold text-card-foreground-dark">{item.name}</Text>
              ...
              <View className="mt-3 flex-row items-center gap-2">
                ...
                <Text className="min-w-9 text-center text-base font-semibold text-foreground-dark">
                  {item.quantity}
                </Text>
